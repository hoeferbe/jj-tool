...
Für ein Update muss derselbe Keystore verwendet werden; der Keystore und die vier Actions-Secrets (`ANDROID_KEYSTORE_BASE64`, `ANDROID_KEYSTORE_PASSWORD`, `ANDROID_KEY_ALIAS`, `ANDROID_KEY_PASSWORD`) dürfen nicht ins Repository gelangen. Vor dem ersten Release müssen alle vier Secrets im Repository eingerichtet sein. Der Release-Workflow erkennt den Keystore-Typ (`PKCS12` oder `JKS`) zuerst direkt aus der Datei und prüft danach nur noch den erkannten Typ mit dem hinterlegten Passwort. Dabei normalisiert er gängige Secret-Formate wie Zeilenumbrüche, umschließende Anführungszeichen und `data:...;base64,`-Präfixe, damit valide Keystores zuverlässig dekodiert werden. So hängt die Typ-Erkennung nicht mehr von `keytool` ab, fehlerhafte Secrets werden aber weiterhin früh mit einer präzisen Meldung zurückgewiesen. Bei `PKCS12` verwendet Android intern das Keystore-Passwort auch für den privaten Schlüssel; ein abweichendes `ANDROID_KEY_PASSWORD` ist dort wirkungslos und würde den Signatur-Build scheitern lassen. Wenn das Passwort lokal in `zsh` geprüft wird und ein `!` enthält, muss der Aufruf mit einfachen Anführungszeichen erfolgen oder das `!` als `\!` escaped werden, zum Beispiel `keytool -list -keystore jagdrevier.keystore -storepass 'm3!nj@gdrevier'`.
